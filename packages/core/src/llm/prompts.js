...
exports.registerPromptVersion = registerPromptVersion;
 ...
 */
function getPrompt(role, version) {
 ...
 */
function getCurrentVersion(role) {
    return CURRENT_VERSIONS[role];
}
/**
 * Get all available versions for a role.
 */
function getAvailableVersions(role) {
 ...
 */
function registerPromptVersion(role, version, prompt) {
    PROMPT_VERSIONS[role].set(version, prompt);
}
/**
 * Set the current version for a role.
 */
function setCurrentVersion(role, version) {
