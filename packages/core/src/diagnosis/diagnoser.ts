...
export class Diagnoser {
  ...
  private get diagnosisTimeoutMs(): number {
    return this.config.diagnosisTimeoutMs ?? DEFAULT_DIAGNOSIS_CONFIG.diagnosisTimeoutMs;
  }

  /**
   * Diagnose a failure and identify root cause.
   */
  async diagnose(context: FailureContext): Promise<DiagnosisResult> {
   ...
   */
  private buildDiagnosisPrompt(context: FailureContext): string {
...
Provide your analysis in structured format with root_cause, summary, analysis, and potential_fixes.`;
   ...
   */
  private identifyRootCause(context: FailureContext): { category: RootCauseCategory; confidence: number } {
  ...
  private identifyPotentialFixes(
    context: FailureContext,
    rootCause: { category: RootCauseCategory; confidence: number }
  ): PotentialFix[] {
    const fixes: PotentialFix[] = [];
   ...
   */
  private findRelatedPatterns(context: FailureContext): string[] {
    const patterns: string[] = [];

    // Check for repeated failures
    const failureEvents = context.recentEvents.filter(e =>
    ...
    const stateChanges = context.recentEvents.filter(e =>
      e.type.includes('STATE_CHANGE')
    );
   ...
   */
  private generatePreventionRecommendations(category: RootCauseCategory): string[] {
