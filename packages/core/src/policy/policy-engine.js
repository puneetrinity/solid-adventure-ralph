...
exports.DEFAULT_POLICY_CONFIG = void 0;
 ...
 */
function extractAddedLines(diff, filePath) {
 ...
 */
function evaluatePolicy(diff, config = exports.DEFAULT_POLICY_CONFIG) {
    const violations = [];
    const parsed = (0, diff_parser_1.parseDiff)(diff);
    for (const file of parsed.files) {
        ...
        const isDependencyFile = config.dependencyFiles.some((depFile) => file.path === depFile || file.path.endsWith(`/${depFile}`));
        if (isDependencyFile) {
            violations.push({
...
function createPolicyConfig(overrides = {}) {
    return {
        frozenFiles: overrides.frozenFiles ?? exports.DEFAULT_POLICY_CONFIG.frozenFiles,
        denyGlobs: overrides.denyGlobs ?? exports.DEFAULT_POLICY_CONFIG.denyGlobs,
        secretPatterns: overrides.secretPatterns ?? exports.DEFAULT_POLICY_CONFIG.secretPatterns,
        dependencyFiles: overrides.dependencyFiles ?? exports.DEFAULT_POLICY_CONFIG.dependencyFiles,
        allowDependencyChanges: overrides.allowDependencyChanges ?? exports.DEFAULT_POLICY_CONFIG.allowDependencyChanges,
    };
}
