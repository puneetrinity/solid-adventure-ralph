...
export class BackendAgent extends BaseAgent {
  ...
  async validate(context: AgentContext): Promise<AgentValidationResult> {
    ...
    const hasBackendFiles = task.targetFiles.some(file =>
      backendKeywords.some(kw => file.toLowerCase().includes(kw)) ||
  ...
  async propose(context: AgentContext): Promise<ProposalResult> {
    ...
    try {
      ...
      return this.createFailure(
        `Backend agent failed: ${(error as Error).message}`,
        startTime
      );
    }
  }
  ...
  private buildBackendPrompt(context: AgentContext): string {
...
Respond with the code changes needed.`;
    ...
    const patchSet: PatchSetProposal = {
      ...
      patches: [
        this.createPatch(
          ...
          `Backend changes for ${context.task.type}: ${context.task.description}`,
          diff,
          files,
          {
  ...
  private generateBackendStubDiff(context: AgentContext): string {
...
+
 export class Service {
  ...
  private assessRiskLevel(context: AgentContext): 'low' | 'medium' | 'high' {
