...
export class TestAgent extends BaseAgent {
  ...
  async validate(context: AgentContext): Promise<AgentValidationResult> {
    ...
    const hasTestFiles = task.targetFiles.some(file =>
  ...
  async propose(context: AgentContext): Promise<ProposalResult> {
    ...
    try {
      ...
      return this.createFailure(
        `Test agent failed: ${(error as Error).message}`,
        startTime
      );
    }
  }
  ...
  private buildTestPrompt(context: AgentContext): string {
...
Respond with the test code needed.`;
  ...
  private generateStubProposal(
    ...
    const files: AgentPatchFile[] = testFiles.map(path => ({
      path,
    ...
    const patchSet: PatchSetProposal = {
      ...
      patches: [
        this.createPatch(
          context.task.id,
          `Add tests for ${context.task.title}`,
          `Test implementation: ${context.task.description}`,
          diff,
          files,
          {
            riskLevel: 'low',
  ...
  private determineTestFiles(targetFiles: string[]): string[] {
  ...
  private generateTestStubDiff(context: AgentContext, testFile: string): string {
...
+});`;
  ...
  private getRelativeImport(testFile: string, sourceFile: string): string {
