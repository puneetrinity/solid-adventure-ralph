...
class ReviewAgent extends base_agent_1.BaseAgent {
    ...
    async propose(context) {
        ...
        try {
            ...
            return this.createFailure(`Review agent failed: ${error.message}`, startTime);
        }
    }
...
Respond with the code improvements as patches.`;
    ...
    generateStubProposal(context, startTime) {
        ...
        const patchSet = {
            ...
            patches: [
                this.createPatch(context.task.id, `Review improvements for ${context.task.title}`, `Code quality improvements based on review findings:\n${review.recommendations.map(r => `- ${r}`).join('\n')}`, diff, files, {
    ...
    generateReviewDiff(context, review) {
...
+  return data.map(item => (item as Record<string, unknown>).value);
    ...
    determineRiskFromFindings(findings) {
